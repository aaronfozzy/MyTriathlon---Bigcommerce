$(document).ready(function(){
	if($(".complete-the-look-widget").length){
		...
		$.ajax(settings).done(function (response) {
			...
			var noOfProducts = response[0]['lineItems']['physicalItems']['length'];
			...
			for (var i = 0, l = cartData.length; i < l; i++) {
				...
				$.post("https://mmsapp.mytriathlon.co.uk/ajax/website-functions/complete-the-look.php", {
					...
					if(response == "Not Permitted" || response == "Please check the request no product id found" || response == "Curl Error"){
						...
						$(".complete-the-look-widget .products-holder").prepend(completeTheLookMarkup);
						
						console.log(completeTheLookMarkup);
						
						if(noOfProducts >= 1){
							if(completeTheLookMarkup.length > 100){
								$(".complete-the-look-widget").show();
							}
						}
					}
				});  
